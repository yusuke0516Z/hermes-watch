...
2. **ScrapFly経由（有料・`scrapfly.enabled`）**
   ...
   **実証済み（2026-08-13）**:
   ...
   運用設定（2026-08-13 $30プラン契約後）: **全時間帯を有料経路に**。
   深夜0〜8時は5分間隔・日中は15分間隔（cronそのまま）、`url_limit: 1` で1回1ページ。
   → **168回/日 × 30クレジット = 月約151,200 / 200,000**（バッファ約4.4万）。
   **24時間リアルタイム監視**になっている。間隔を詰める時は必ず試算をやり直すこと
   （5分×24時間は月259,200で枠超過）。
