...
2. **ScrapFly経由（有料・`scrapfly.enabled`）**
   ...
   費用の目安: asp+住宅IPで約25-30クレジット/回、$30プランで20万クレジット/月。
   5分間隔24時間だと約26万クレジットで足が出るため、`hours_jst` で**補充が集中する
   JST 0〜8時だけ課金**する設定にしてある（約10万クレジット/月）。
   有料経路が落ちても自動的に無料経路へフォールバックする。
