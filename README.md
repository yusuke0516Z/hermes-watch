...
**原因の切り分け**: `hermes_monitor.py` の `run_loop`（`budget_seconds=1680`・`day_interval_sec=900`）は、
昼間帯は必ず2イテレーション（0分・15分時点）で打ち切られる構造になっている
（3回目を仕込むには `budget_seconds` を約1980秒＝33分まで伸ばす必要があり、それだと
cronの30分周期をまたいで次runと重なる）。つまり現状の設定でも「28分ループ」は昼間は
実質15〜16分しか使えておらず、これは既知の制約（`day_interval_sec` を割り込む前提上の限界）。
一方で観測された47〜116分という間隔は、この15〜16分の消化だけでは説明できない大きさで、
**GitHub Actions の `schedule` トリガー自体が想定より高い頻度で遅延・間引かれている**ことが
主因と見られる（`:13,:43` オフセット済みでも解消していない）。

**今回コードを変更しなかった理由**: このリポジトリの制約（監視間隔を狭めない・大きな設計変更をしない）
の範囲では、`budget_seconds` や `margin` を調整しても上記の理由で日中の実効カバレッジを
安全に伸ばせない。gapの主因がGitHub側のスケジューラである以上、ワークフロー設定の小手先の
調整では再現性のある改善が見込めないと判断した。

**次に検討する価値がありそうな案**（設計変更を伴うため、実施は判断者の承認を得てから）:

1. 自分のPC（または安価なVPS/Cloud Run等）で `hermes_monitor.py --loop` を常駐実行し、
   GitHub Actionsは死活監視・バックアップ経路に格下げする。cron依存を根本的になくせる。
2. GitHub Actionsを使い続ける場合、`schedule` の代わりに外部cronサービス（cron-job.org等）から
   `workflow_dispatch` をAPI経由で叩く方式に変える（GitHub謹製cronより信頼性が高いとの報告がある）。
3. 現状のままなら、鮮度の限界（README冒頭の「鮮度の限界」節）に「昼間はcron遅延により
   実測1〜2時間ずれることがある」と明記し、期待値を調整する。
