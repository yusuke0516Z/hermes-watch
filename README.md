...
2. **ScrapFly経由（有料・`scrapfly.enabled`）**
   ...
   **実証済み（2026-08-13）**:
   ...
   `render_js: false` でも описание5クレジットは発生する）。

   運用設定: `hours_jst` で**補充が集中する JST 0〜8時だけ課金**し、`url_limit: 1` で
   1回1ページに絞る → **108回/日 × 30 = 約97,200クレジット/月**（$30プラン20万の約半分）。
   この時間帯はworkflowのcron（`*/5 15-23 UTC` = JST 0:00-8:55）と一致させてある。
   日中は無料経路（2ページ併用）で動く。
